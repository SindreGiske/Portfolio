...
export default function HeroSection() {
  ...
  const HeroLink = ({ name, href, children }: HeroLinkProps) => {
    return (
      <a target="_blank" href={href} rel="noopener noreferrer" aria-label={name} className="transition hover:scale-110">
        {children}
      </a>
    );
  };
  ...
  return (
    <section ref={ref} id={"hero"} className="relative h-[90vh] w-[99%] overflow-hidden">
      <div className="relative top-12 z-10 flex h-full w-full flex-col-reverse items-center justify-center gap-3 md:flex-row">
        <div
          className={`font-bold, flex h-fit w-fit items-start gap-2 text-2xl text-amber-300 transition-all delay-[2500ms] duration-600 ease-in md:flex-col md:text-4xl ${
        ...
        <div className="flex h-fit w-fit flex-col items-center gap-3 md:items-start">
          <div className="flex flex-row gap-4 text-center text-3xl font-bold text-amber-300 md:text-5xl">
            ...
            <h1
              className={`transition-all delay-500 duration-600 ease-in ${
                hasAnimated ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
              }`}
            >
              TOFTE
            ...
            <h1
              className={`transition-all delay-1000 duration-600 ease-in ${
                hasAnimated ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
              }`}
            >
              GISKE
          ...
          <h2
            className={`font-bold, text-2xl text-amber-300 transition-all delay-[2000ms] duration-600 ease-in md:text-4xl ${
              hasAnimated ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
            }`}
          >
            Full-Stack Utvikler
